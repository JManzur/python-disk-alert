...
def disk_var():
    for p in psutil.disk_partitions():
        if p.device == 'C:\\':
            devices = {}
            disk_free_gb_long = psutil.disk_usage(p.mountpoint).free / 1024 / 1024 / 1024
            disk_free_gb_short = "{:.2f}".format(disk_free_gb_long)
            disk_used_percent = psutil.disk_usage(p.mountpoint).percent
            devices[p.device] = disk_used_percent
            file_system = p.fstype
            drive = p.device
            return drive, file_system, disk_used_percent, disk_free_gb_short
    ...
    for p in psutil.disk_partitions():
        if p.device == 'C:\\':
            ...
            if disk_used_percent > threshold:
                send_email()
                logging.warning('Drive: "{0:}", Usage: {1:}%, Free: {2:} GiB, File System: {3:}.'.format(p.device, disk_used_percent, disk_free_gb_short, file_system))
                ...
                logging.info('Drive: "{0:}", Usage: {1:}%, Free: {2:} GiB, File System: {3:}.'.format(p.device, disk_used_percent, disk_free_gb_short, file_system))
                logging.info('OK - Drive "{0:}" has a usage percentage less than the defined threshold of {1:}%'.format(p.device, threshold))

# Send Mail funtion
def send_email():
    drive, file_system, disk_used_percent, disk_free_gb_short=disk_var()
    msg = EmailMessage()
    ...
    Used Percentage: {6:}%
    Total Free: {7:} GiB
    """.format(threshold, now_date, hostname, ip_address, drive, file_system, disk_used_percent, disk_free_gb_short))
